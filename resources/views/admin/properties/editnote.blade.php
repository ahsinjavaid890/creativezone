...
<div class="content d-flex flex-column flex-column-fluid" id="kt_content">
    ...
    <div class="d-flex flex-column-fluid">
        ...
        <div class=" container-fluid ">
            ...
            <form method="POST" action="{{ url('admin/properties/updatenote') }}">
                @csrf
                                        ...
                                        <div class="row">
                                            <div class="col-md-12">
                                                <div class="form-group">
                                                    ...
                                                    <select class="form-control" name="visit_type" id="visit_type" >
                                                        ...
                                                        <option @if($note->visit_type == 'email') selected @endif value="email">Email</option>
                                                        <option @if($note->visit_type == 'call') selected @endif value="call">Call</option>
                                                        <option @if($note->visit_type == 'visit') selected @endif value="visit">Visit</option>
                                                    </select>
                                                </div>
                                            </div>
                                            <div class="col-md-12">
                                            ...
                                            <div class="col-md-12">
                                                <div class="rating">
                                                    <div class="rating-component">
                                                        ...
                                                        <div class="stars-box">
                                                            @for ($i = 1; $i <= 5; $i++)
                                                                <i class="star fa fa-star  {{ $note->rating >= $i ? 'selected' : '' }}" title="{{ $i }} star" data-message="{{ $i == 1 ? 'Poor' : ($i == 2 ? 'Bad' : ($i == 3 ? 'Average Quality' : ($i == 4 ? 'Nice' : 'Very Good Quality'))) }}" data-value="{{ $i }}"></i>
                                                            @endfor
                                                        </div>
                                                        <div class="starrate">
                                                            <label>
                                                                <input class="ratevalue" type="hidden" name="rating" value="{{ $note->rating }}" >
                                                            </label>
                                                        </div>
                                                    </div>
                                                </div>
                                            ...
                                            <div class="col-md-12">
                                                <div class="form-group">
                                                    ...
                                                    <select id="parent_service" name="parentservice" class="form-control" onchange="getprovider(this.value)">
                                                        ...
                                                        dd($r->id);
                                                        <option @if($note->parentservice == $r->id) selected @endif  value="{{ $r->id }}">{{ $r->name }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            </div>
                                            <div class="col-md-12">
                                                <div class="form-group">
                                                    ...
                                                    <select id="showprovider" name="provider" class="form-control" onchange="selecttempalte(this.value)" >
                                                       <option value="" >Select All provider</option>
                                                        @php
                                                            $providers = DB::table('providers')
                                                                            ->where('service', $note->parentservice)
                                                                            ->get();
                                                        @endphp

                                                        @foreach($providers as $r)
                                                            <option @if($note->provider == $r->id) selected @endif value="{{ $r->id }}">{{ $r->name }}</option>
                                                        @endforeach
                                                    </select>
                                                </div> 
                                            </div>
                                            <div class="col-md-12">
                                                <div class="form-group">
                                                    ...
                                                    <select class="form-control" name="emailtemplate" id="emailtemplate" onchange="selecttemplatedata(this.value)" >
                                                        <option value="">Select Email Templates</option>
                                                        @foreach(DB::table('emailtemplates')->where('provider_id' , $note->provider)->get() as $r)
                                                        <option @if($note->emailtemplate == $r->id) selected @endif value="{{ $r->id }}">{{ $r->name }}</option>
                                                        @endforeach
                                            ...
                                            <div class="col-md-12">
                                                <div class="form-group">
                                                    ...
                                                    @else
                                                        <select class="form-control" name="emailto" id="emailto" onchange="getnewemail(this.value)">
                                                            <option value="">Select Email Templates</option>
                                                            <option @if($note->emailto == $property->id) selected @endif value="{{ $property->id }}">{{ $property->office_email }}</option>
                                            ...
                                            <div class="col-md-12">
                                                 <div class="form-group">
                                                    <label>Subject</label>
                                                    <input type="text" name="subject" value="{{ $note->subject }}" id="emailsubject" class="form-control" >
                                                </div>
                                            </div>
                                            <div class="col-md-12">
                                                <div class="form-group">
                                                    <label>Email Template Description</label>
                                                    <textarea class="summernote" name="emaildescription" >
                                                        {{ $note->emaildescription }}
                                                    </textarea>
                                                </div>
                                            </div>
                                            <div class="col-md-12">
                                                <div class="form-group">
                                                    ...
                                                    <select class="form-control" name="attachments" id="attachments" >
                                                        ...
                                                        <option @if($note->attachments == $r->id) selected @endif value="{{ $r->id }}">{{ $r->name }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            </div>
...
$(document).ready(function() {
    $('.star').on('click', function() {
        ...
        $('.ratevalue').val(ratingValue);
    });

    // Ensure the initial star rating is highlighted
    var initialRating = $('.ratevalue').val();
    $('.stars-box .star').each(function() {
...
function getnewemail(id) {
  ...
  $(document).ready(function() {
        let selectemail = $('#emailto').val();
        if (selectemail) {
            getnewemail(selectemail);
        }
    });
